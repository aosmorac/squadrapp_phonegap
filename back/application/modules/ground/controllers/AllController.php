...
class AllController extends Zend_Controller_Action
{
	...
	public function searchFieldsAction ()
    {
            ...
            $city_id = 0;
            $sport = trim($this->getParam('sport'));
            $sports = array("fútbol"=>"fútbol", "futbol"=>"fútbol", "soccer"=>"fútbol", "football"=>"fútbol");
            if (isset($sports[mb_strtolower($sport,"UTF-8")]))
                $sport = $sports[mb_strtolower($sport,"UTF-8")];    //  Crear clase estatica.
            $sport_id = 0;
            
            $city_model = new Model_DbTable_City();
            $sport_model = new Model_DbTable_Sport();
            $city_info = $city_model->getCityByName($city)->toArray();
            $sport_info = $sport_model->getSportByName($sport)->toArray();
            ...
            if (!isset($sport_info[0])){
                ...
                $branches = $branch_model->getBranches($city_id, $sport_id);
	...
	public function saveSuggestSportAction ()
        {
            $this->_helper->layout()->disableLayout ();
            $this->_helper->viewRenderer->setNoRender(true);
            $other_sport = $this->getParam('sport');
            $suggestions = new Model_Suggestions();
